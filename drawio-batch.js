...
program
  ...
  .option('-o --outputDir <directory>',
    'selects a output directory', parseDirectory, 0)
  .arguments('<input...>')
...
(async () => {
  ...
  try {
    ...
    for (let file of input) {
      ...
      const doc = new xmldom.DOMParser().parseFromString(await fileContent);
      ...
      for (let diagramId = 0; diagramId < diagrams.length; diagramId++) {
        ...
        }, fileContent, program.format, program.bounds, program.scale, diagramId)
        ...
        const output = program.outputDir;
        ...
        if (extension === 'pdf') {
          ...
          var domText = await page.evaluate(() => {
            ...
            console.log(svgElement.parentElement.innerHTML);
